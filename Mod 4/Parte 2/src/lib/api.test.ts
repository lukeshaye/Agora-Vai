 ...
 * mockando o 'fetch' subjacente conforme o plano, garantindo que o cliente 'api'
 * está configurado para chamar o endpoint correto.
 */
...
describe('API Client (Hono RPC)', () => {
  ...
  it('deve exportar a instância do cliente Hono RPC', () => {
    ...
    // O objeto 'api' deve ter as propriedades de um cliente Hono, como um método '$get'
    // ou acesso a rotas aninhadas.
  ...
  it('deve fazer uma chamada GET com o prefixo "/api" (DIP, SoC)', async () => {
    ...
    // O Hono Client cria rotas baseadas na tipagem, mas para fins de teste de infra,
    // usamos uma chamada RPC simples.
    // Nota: O tipo 'api' é inferido de forma incompleta, por isso o cast para 'any'.
    ...
    // Esperamos que a chamada seja para '/api' + a rota RPC (ex: /index)
    const calledUrl = mockFetch.mock.calls[0][0];

    // O teste verifica o prefixo básico. O nome da rota pode variar.
    expect(calledUrl).toMatch(/^\/api\/.*/);

    // 4. Verifica se o método HTTP é GET (método padrão para $get())
